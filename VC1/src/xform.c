		     ...
		     struct feature*** inliers, int* n_in )
{
  ...
  if (nm < m) //若找到的具有匹配点的特征点个数小于计算变换矩阵需要的最小特征点对个数，出错
    {
  ...
  rand(time(NULL));//初始化随机数生成器

  in_min = calc_min_inliers(nm, m, RANSAC_PROB_BAD_SUPP, p_badxform); //计算保证RANSAC最终计算出的转换矩阵错误的概率小于p_badxform所需的最小内点数目
  p = pow(1.0 - pow(in_frac, m), k); //当前计算出的模型的错误概率,内点所占比例in_frac越大，错误概率越小；迭代次数k越大，错误概率越小
  while (p > p_badxform) //当前错误概率大于输入的允许错误概率p_badxform，继续迭代
    {
	  sample = draw_ransac_sample(matched, nm, m); //从样本集matched中随机抽选一个RANSAC样本(即一个4个特征点的数组)，放到样本变量sample中
      extract_corresp_pts( sample, m, mtype, &pts, &mpts );//从样本中获取特征点和其对应匹配点的二维坐标，分别放到输出参数pts和mpts中
	  M = xform_fn(pts, mpts, m); //调用参数中传入的函数xform_fn，计算将m个点的数组pts变换为mpts的矩阵，返回变换矩阵给M
      if( ! M )//出错判断
	goto iteration_end;
	  in = find_consensus(matched, nm, mtype, M, err_fn, err_tol, &consensus); //给定特征点集，变换矩阵，误差函数，计算出当前一致集consensus，返回一致集中元素个数给in
	  if (in > in_max) //若当前一致集大于历史最优一致集，即当前一致集为最优，则更新最优一致集consensus_max
	{
  ...
  //根据最优一致集计算最终的变换矩阵
  if (in_max >= in_min) //若最优一致集中元素个数大于最低标准，即符合要求
    {
      extract_corresp_pts( consensus_max, in_max, mtype, &pts, &mpts );//从最优一致集中获取特征点和其对应匹配点的二维坐标，分别放到输出参数pts和mpts中
      M = xform_fn( pts, mpts, in_max );//调用参数中传入的函数xform_fn，计算将in_max个点的数组pts变换为mpts的矩阵，返回变换矩阵给M
	  /***********下面会再进行一次迭代**********/
	  ...
	  M = xform_fn(pts, mpts, in); //调用参数中传入的函数xform_fn，计算将in个点的数组pts变换为mpts的矩阵，返回变换矩阵给M
      if( inliers )
	{
  ...
  for( i = 0; i < nm; i++ )
    {
      rdata = feat_ransac_data( matched[i] );//利用宏feat_ransac_data来提取matched[i]中的feature_data成员并转换为ransac_data格式的指针
	  matched[i]->feature_data = rdata->orig_feat_data; //恢复feature_data成员的以前的值
      free( rdata );
    }
  free( matched );
  return M;//返回求出的变换矩阵M
