		    ...
		    int img_dbl, int descr_width, int descr_hist_bins )
{
  ...
  octvs = log(MIN(init_img->width, init_img->height)) / log(2) - 2; //计算高斯金字塔的组数octvs
  ...
  if( img_dbl )//若设置了将图像放大为原图的2倍
    adjust_for_img_dbl( features );//将特征点序列中每个特征点的坐标减半(当设置了将图像放大为原图的2倍时，特征点检测完之后调用)
...
static IplImage* create_init_img( IplImage* img, int img_dbl, double sigma )
{
  ...
  gray = convert_to_gray32( img );//调用函数，将输入图像转换为32位灰度图，并归一化
  if (img_dbl) //若设置了将图像放大为原图的2倍
    {
	  //将图像长宽扩展一倍时，便有了底-1层，该层尺度为：
	  //sigma是第0层尺度，SIFT_INIT_SIGMA为被相机镜头模糊后的尺度
	  /*在检测极值点前对原始图像的高斯平滑以致图像丢失高频信息，所以
		Lowe建议在建立尺度空间前首先对原始图像长宽扩展一倍，以保留原始图像
		信息，增加特征点数量。当对图像长宽扩展一倍时，便构建了-1层，该层尺度为*/
      sig_diff = sqrt( sigma * sigma - SIFT_INIT_SIGMA * SIFT_INIT_SIGMA * 4 );
      dbl = cvCreateImage( cvSize( img->width*2, img->height*2 ),//创建放大图像
			   IPL_DEPTH_32F, 1 );
      cvResize( gray, dbl, CV_INTER_CUBIC );//放大原图的尺寸
      cvSmooth( dbl, dbl, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff );//高斯平滑，高斯核在x,y方向上的标准差都是sig_diff
      cvReleaseImage( &gray );
      return dbl;
    }
  else//不用放大为原图的2倍
    {
	  sig_diff = sqrt(sigma * sigma - SIFT_INIT_SIGMA * SIFT_INIT_SIGMA); // 计算第0层的尺度
      cvSmooth( gray, gray, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff );//高斯平滑，高斯核在x,y方向上的标准差都是sig_diff
      return gray;
    }
}
...
static IplImage* convert_to_gray32( IplImage* img )
{
  ...
  cvConvertScale( gray8, gray32, 1.0 / 255.0, 0 );//然后将8为单通道图像gray8转换为32位单通道图像，并进行归一化处理(除以255)
			     ...
			     int intvls, double sigma )
{
  ...
  gauss_pyr = calloc( octvs, sizeof( IplImage** ) );//为高斯金字塔gauss_pyr分配空间，共octvs个元素，每个元素是一组图像的首指针
  for( i = 0; i < octvs; i++ )//为第i组图像gauss_pyr[i]分配空间，共intvls+3个元素，每个元素是一个图像指针
  ...
  */
  ...
  //计算每次高斯模糊的sigma参数
  sig[0] = sigma;//初始尺度
  sig[1] = sigma * sqrt( k*k- 1 );//不懂这是为什么？？？？？？？？？？？？？？？
  ...
  for( o = 0; o < octvs; o++ )//遍历组
	...
	else if( i == 0 )//新的一组的首层图像是由上一组最后一层(倒数第三层)图像下采样得到
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
  ...
  dog_pyr = calloc( octvs, sizeof( IplImage** ) );//为高斯差分金字塔分配空间，共octvs个元素，每个元素是一组图像的首指针
  for (i = 0; i < octvs; i++) //为第i组图像dog_pyr[i]分配空间，共(intvls+2)个元素，每个元素是一个图像指针
				   ...
				   CvMemStorage* storage )
{
  ...
  double prelim_contr_thr = 0.5 * contr_thr / intvls;//像素的对比度阈值（不懂为什么？？？？？？？？？）
  struct feature* feat;
					...
					double contr_thr )
{
  ...
  while( i < SIFT_MAX_INTERP_STEPS )//SIFT_MAX_INTERP_STEPS指定了关键点的最大插值次数，即最多修正多少次，默认是5
    {
      interp_step( dog_pyr, octv, intvl, r, c, &xi, &xr, &xc );//进行一次极值点差值，计算σ(层方向,intvl方向)，y，x方向上的子像素偏移量(增量)
      if( ABS( xi ) < 0.5  &&  ABS( xr ) < 0.5  &&  ABS( xc ) < 0.5 )
	break;//若在任意方向上的偏移量大于0.5时，意味着差值中心已经偏移到它的临近点上，所以必须改变当前关键点的位置坐标
      
	  //修正关键点的坐标，x,y,σ三方向上的原坐标加上偏移量取整(四舍五入)
	  c += cvRound(xc); //对一个double型的数进行四舍五入
      r += cvRound( xr );
      intvl += cvRound( xi );
  ...
  //计算被插值点的对比度：D + 0.5 * dD^T * X
  contr = interp_contr(dog_pyr, octv, intvl, r, c, xi, xr, xc); //(xi, xr, xc应为i,r,c?????????????????????)
  if( ABS( contr ) < contr_thr / intvls )//若该点对比度过小，舍弃，返回NULL( contr_thr / intvls应为contr_thr？？？？？？？？？？？？？？)
    return NULL;

  feat = new_feature();//为一个特征点feature结构分配空间并初始化，返回特征点指针
  ddata = feat_detection_data( feat );//调用宏feat_detection_data来提取参数feat中的feature_data成员并转换为detection_data类型的指针
  //将修正后的坐标赋值给特征点feat
...
static CvMat* deriv_3D( IplImage*** dog_pyr, int octv, int intvl, int r, int c )
{
  ...
  dI = cvCreateMat( 3, 1, CV_64FC1 );//组成列向量
  cvmSet( dI, 0, 0, dx );
  cvmSet( dI, 1, 0, dy );
  cvmSet( dI, 2, 0, ds );
			  ...
			  int c )
{
  ...
  dxx = ( pixval32f( dog_pyr[octv][intvl], r, c+1 ) + 
	  pixval32f(dog_pyr[octv][intvl], r, c - 1) - 2 * v); //用差分近似代替导数(具体公式见各种梯度的求法)
  ...
  H = cvCreateMat( 3, 3, CV_64FC1 );//组成海森矩阵
  cvmSet( H, 0, 0, dxx );
  cvmSet( H, 0, 1, dxy );
  cvmSet( H, 0, 2, dxs );
  cvmSet( H, 1, 0, dxy );
			    ...
			    int c, double xi, double xr, double xc )
{
  ...
  cvInitMatHeader(&X, 3, 1, CV_64FC1, x, CV_AUTOSTEP); //偏移量组成的列向量X，其中是x，y，σ三方向上的偏移量
  cvInitMatHeader( &T, 1, 1, CV_64FC1, t, CV_AUTOSTEP );//矩阵乘法的结果T，是一个数值
  dD = deriv_3D( dog_pyr, octv, intvl, r, c );//在DoG金字塔中计算某点的x方向、y方向以及尺度方向上的偏导数，结果存放在列向量dD中
  cvGEMM( dD, &X, 1, NULL, 0, &T,  CV_GEMM_A_T );//矩阵乘法：T = dD^T * X
  cvReleaseMat( &dD );
...
static int is_too_edge_like( IplImage* dog_img, int r, int c, int curv_thr )
{
  ...
  if( det <= 0 )//若行列式为负，表明曲率有不同的符号，去除此点
...
static void adjust_for_img_dbl( CvSeq* features )
{
  ...
  for (i = 0; i < n; i++) //遍历特征点
    {
      feat = CV_GET_SEQ_ELEM( struct feature, features, i );//调用宏，获取序列features中的第i个元素，并强制转换为struct feature类型
	  //将特征点的x,y坐标和尺度都减半
      feat->x /= 2.0;
      feat->y /= 2.0;
      feat->scl /= 2.0;
...
static void calc_feature_oris( CvSeq* features, IplImage*** gauss_pyr )
{
  ...
  for( i = 0; i < n; i++ )//遍历特征点序列
    {
	  feat = malloc(sizeof(struct feature)); //给每个特征点分配feature结构大小的内存
	  cvSeqPopFront(features, feat); //移除列首元素，放到feat中
	  //调用宏feat_detection_data来提取参数feat中的feature_data成员并转换为detection_data类型的指针 
      ...
      hist = ori_hist( gauss_pyr[ddata->octv][ddata->intvl],//特征点所在的图像
		       ddata->r, ddata->c, SIFT_ORI_HIST_BINS,//特征点的行列坐标//默认的梯度直方图的bin(柱子)个数
		       cvRound( SIFT_ORI_RADIUS * ddata->scl_octv ),//特征点方向赋值过程中，搜索邻域的半径为：3 * 1.5 * σ
			   SIFT_ORI_SIG_FCTR * ddata->scl_octv); //计算直翻图时梯度幅值的高斯权重的初始值
	  for (j = 0; j < SIFT_ORI_SMOOTH_PASSES; j++) //对梯度直方图进行高斯平滑，弥补因没有仿射不变性而产生的特征点不稳定的问题,SIFT_ORI_SMOOTH_PASSES指定了平滑次数
	smooth_ori_hist( hist, SIFT_ORI_HIST_BINS );
	  omax = dominant_ori(hist, SIFT_ORI_HIST_BINS); //查找梯度直方图中主方向的梯度幅值，即查找直方图中最大bin的值,返回给omax
	  /*若当前特征点的直方图中某个bin的值大于给定的阈值，则新生成一个特征点并添加到特征点序列末尾
			 ...
			 double sigma )
{
  ...
  hist = calloc(n, sizeof(double)); //为直方图数组分配空间，共n个元素，n是柱的个数
  exp_denom = 2.0 * sigma * sigma;
  //遍历以指定点为中心的搜索区域
  for( i = -rad; i <= rad; i++ )
    for( j = -rad; j <= rad; j++ )
      if( calc_grad_mag_ori( img, r + i, c + j, &mag, &ori ) )//计算指定点的梯度的幅值mag和方向ori，返回值为1表示计算成功
	{
			      ...
			      double* ori )
{
  ...
  if (r > 0 && r < img->height - 1 && c > 0 && c < img->width - 1) //对输入的坐标值进行检查
    {
...
static void smooth_ori_hist( double* hist, int n )
{
  ...
  prev = hist[n-1];
  for( i = 0; i < n; i++ )//类似均值漂移的一种邻域平滑，减少突变的影响
    {
      tmp = hist[i];
				   ...
				   double mag_thr, struct feature* feat )
{
  ...
  for (i = 0; i < n; i++) //遍历直方图
	...
	{
	  bin = i + interp_hist_peak( hist[l], hist[i], hist[r] );//根据左、中、右三个bin的值对当前bin进行直方图插值
...
static struct feature* clone_feature( struct feature* feat )
{
  ...
  new_feat = new_feature(); //为一个feature结构分配空间并初始化
  ddata = feat_detection_data(new_feat); //调用宏feat_detection_data来提取参数feat中的feature_data成员并转换为detection_data类型的指针
  memcpy(new_feat, feat, sizeof(struct feature)); // 对内存空间进行赋值
				 ...
				 int n )
{
  ...
  for( i = 0; i < k; i++ )//遍历特征点序列中的特征点
    {
      feat = CV_GET_SEQ_ELEM( struct feature, features, i );//调用宏，获取序列features中的第i个元素，并强制转换为struct feature类型
      ddata = feat_detection_data( feat );//调用宏feat_detection_data来提取参数feat中的feature_data成员并转换为detection_data类型的指针
      hist = descr_hist( gauss_pyr[ddata->octv][ddata->intvl], ddata->r,//计算特征点附近区域的方向直方图，此直方图在计算特征描述子中要用到，返回值是一个d*d*n的三维数组
			 ddata->c, feat->ori, ddata->scl_octv, d, n );
      hist_to_descr( hist, d, n, feat );//将某特征点的方向直方图转换为特征描述子向量，对特征描述子归一化并将所有元素转化为整型，存入特征点feat中
	  release_descr_hist(&hist, d); //释放特征点的方向直方图
    }
}
...
//为直方图数组分配空间
  ...
  bins_per_rad = n / PI2;
  exp_denom = d * d * 0.5;
  hist_width = SIFT_DESCR_SCL_FCTR * scl;//计算特征描述子过程中，特征点周围的d*d个区域中，每个区域的宽度为m*σ个像素，SIFT_DESCR_SCL_FCTR即m的默认值，σ为特征点的尺度
  ...
  for( i = -radius; i <= radius; i++ )//遍历每个区域的像素
...
static void hist_to_descr( double*** hist, int d, int n, struct feature* feat )
{
  ...
  normalize_descr( feat );//归一化特征点的特征描述子，即将特征描述子数组中每个元素除以特征描述子的模
  for( i = 0; i < k; i++ ) //遍历特征描述子向量，每个元素乘以系数SIFT_INT_DESCR_FCTR来变为整型，并且最大值不能超过255
    if( feat->descr[i] > SIFT_DESCR_MAG_THR )//遍历特征描述子向量，将超过阈值SIFT_DESCR_MAG_THR的元素强行赋值为SIFT_DESCR_MAG_THR
      feat->descr[i] = SIFT_DESCR_MAG_THR;
  normalize_descr( feat );//再次归一化特征描述子向量
