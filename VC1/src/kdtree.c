...
struct kd_node* kdtree_build( struct feature* features, int n )
{
  ...
  kd_root = kd_node_init(features, n); //调用函数，用给定的特征点集初始化k-d树节点，返回值作为树根
  expand_kd_node_subtree( kd_root );//调用函数，扩展根节点kd_root及其左右孩子
		    ...
		    struct feature*** nbrs, int max_nn_chks )
{
  ...
  while( min_pq->n > 0  &&  t < max_nn_chks )//min_pq队列没有回溯完且未达到搜索最大次数
    {
	  ...
	  if (!expl) //出错处理
	{
      ...
      expl = explore_to_leaf( expl, feat, min_pq );//从当前搜索节点expl一直搜索到叶子节点，搜索过程中将未搜索的节点根据优先级放入队列，返回值为叶子节点
	  if (!expl) //出错处理
	{
	  fprintf( stderr, "Warning: PQ unexpectedly empty, %s line %d\n",
		   __FILE__, __LINE__ );
  ...
  for( i = 0; i < n; i++ )//对于最近邻数组中的特征点，恢复其feature_data域的值
    {
...
static void expand_kd_node_subtree( struct kd_node* kd_node )
{
  ...
  assign_part_key( kd_node );//调用函数，确定节点的枢轴索引和枢轴值
  partition_features( kd_node );//在指定k-d树节点上划分特征点集(即根据指定节点的ki和kv值来划分特征点集)
...
static void assign_part_key( struct kd_node* kd_node )
{
  ...
  for( j = 0; j < d; j++ )
    {
      mean = var = 0;
      for( i = 0; i < n; i++ )//求第j维的均值
	mean += features[i].descr[j];
      mean /= n;
	  for (i = 0; i < n; i++) //求第j维的方差
	{
  ...
  kv = median_select( tmp, n );//调用函数，找tmp数组的中值
  free( tmp );
...
static double rank_select( double* array, int n, int r )
{
  ...
  insertion_sort( tmp, rem_elts );//最后一组

  //找中值的中值
  /* recursively find the median of the medians of the groups of 5 */
  tmp = calloc( gr_tot, sizeof( double ) );
  for( i = 0, j = 2; i < gr_5; i++, j += 5 )//将每个5元组中的中值(即下标为2,2+5,...的元素)复制到temp数组
    tmp[i] = array[j];
  if (rem_elts) //最后一组的中值
    tmp[i++] = array[n - 1 - rem_elts/2];
  med = rank_select(tmp, i, (i - 1) / 2); //找temp中的中值med，即中值的中值
  free( tmp );
...
static void partition_features( struct kd_node* kd_node )
{
  ...
  kv = kd_node->kv;//枢轴的值
  for( i = 0; i < n; i++ )
    if( features[i].descr[ki] <= kv )//若第i个特征点的特征向量的第ki维的值小于kv
      {
	tmp = features[++j];
  ...
  tmp = features[p];//将枢轴features[p]和最后一个小于枢轴的点features[j]对换
  features[p] = features[j];
  features[j] = tmp;
  //此后，枢轴的位置下标为j
  /* if all records fall on same side of partition, make node a leaf */
  if( j == n - 1 )//若所有特征点落在同一侧，则此节点成为叶节点
    {
      kd_node->leaf = 1;
      return;
    }

  kd_node->kd_left = kd_node_init( features, j + 1 );//初始化左孩子的根节点，左孩子共j+1个特征点
  kd_node->kd_right = kd_node_init( features + ( j + 1 ), ( n - j - 1 ) );//初始化右孩子的根节点，右孩子共n-j-1个特征点
}
...
{
  struct kd_node* unexpl, * expl = kd_node;//unexpl中存放着优先级队列的候选节点(还未搜索的节点)，expl为当前搜索节点
  double kv;//分割枢轴的值
  int ki;//分割枢轴的维数索引

  while (expl  &&  !expl->leaf) //一直搜索到叶子节点，搜索过程中将未搜索的节点根据优先级放入队列
    {
      ki = expl->ki;
      kv = expl->kv;
      
	  if (ki >= feat->d) //枢轴的维数索引大于特征点的维数，出错
	{
	  fprintf( stderr, "Warning: comparing imcompatible descriptors, %s" \
		   " line %d\n", __FILE__, __LINE__ );
	  return NULL;
	}
      if( feat->descr[ki] <= kv )//目标特征点ki维的数据小于等于kv，进入左子树搜索
	{
				  ...
				  int n, int k )
{
  ...
  dn = ndata->d;
  if (df >= dn) //df>=dn，说明要插入在最近邻数组的末尾
    {
      if( n == k )//最近邻数组中元素个数已达到最大值，无法插入
	{
  ...
  if (n < k) //运行到此处说明插入位置不在数组末尾
    ...
    {
      nbrs[n-1]->feature_data = ndata->old_data;//其实也不是无法插入，而是最近邻数组中元素个数不变，但值会更新
      free( ndata );
    }
  i = n-2;
  while (i >= 0) //在最近邻数组中查找要插入的位置
    {
